...
}`))}function Pt(t){return t.pattern.map(e=>Math.round(e*t.pixelRatio))}function fr(t){if(t==null)return 1;const e=Pt(t);return Math.floor(e.reduce((i,r)=>i+r))}function mr(t){return Pt(t).reduce((e,i)=>Math.max(e,i))}function vr(t){return t==null?ti:t.length===4?t:sr(gr,t[0],t[1],t[2],1)}const gr=ii();function Sr(t,e){if(!e.stippleEnabled)return void t.fragment.code.add(p`float getStippleAlpha() { return 1.0; }
...
}`),a.uniforms.add(new di("stipplePatternTexture",s=>s.stippleTexture),new Y("stipplePatternSDFNormalizer",s=>Tr(s.stipplePattern)),new Y("stipplePatternPixelSizeInv",s=>1/$t(s))),e.stippleOffColorEnabled&&a.uniforms.add(new Be("stippleOffColor",s=>vr(s.stippleOffColor))),a.code.add(p`float getStippleSDF(out bool isClamped) {
...
return isClamped ? floor(antiAliasedResult + 0.5) : antiAliasedResult;
  ...
  `)}function Tr(t){return t?(Math.floor(.5*(mr(t)-1))+.5)/t.pixelRatio:1}function $t(t){const e=t.stipplePattern;return e?fr(t.stipplePattern)/e.pixelRatio:1}const _r=.4,wt=64,br=wt/2,Or=br/5,yr=wt/Or,Ra=.25;function Er(t,e){const i=t.vertex;Xe(i),i.uniforms.get("markerScale")==null&&i.constants.add("markerScale","float",1),i.constants.add("markerSizePerLineWidth","float",yr).code.add(p`float getLineWidth() {
